...
import {asNameMap, flagMap, returnCountryCode} from "./popup_utilities.js";
...
function updatePathUsageVisuals(pathUsage: PerDomainPathUsage) {
    ...
    pathUsageISDs.innerHTML = [...isds].map((isd: number) => `
        <div class="flex flex-row space-x-2">
            <img style="height: 25px" src=${flagMap[returnCountryCode(isd)]} alt=""/>
...
async function openOptionsButtonOnClick() {
    await browser.tabs.create({'url': 'chrome://extensions/?options=' + browser.runtime.id});
}